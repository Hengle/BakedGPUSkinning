...
public class GPUSkinSampler_Ori : MonoBehaviour
{
    ...
    void StartSampleClip(short clipIdx)
    {
        ...
        {
            boneData.matrixes[_sampleParam.clipIdx] = new PosRot[_sampleParam.frameCount];
        }
    ...
    private void GetSampleFrameData()
    {
        ...
        Vector3 tmpPos, tmpScale;
        Quaternion tmpRot;
        foreach (var boneData in _allBoneDatas)
        {
            // 要求物体必须位于根节点并且无旋转无偏移, 否则就要自己逐级乘了
            Matrix4x4 localToWorld = boneData.transform.localToWorldMatrix;
            localToWorld = localToWorld * boneData.bindPose;
            Matrix4x4Helper.Decompose(localToWorld, out tmpScale, out tmpRot, out tmpPos);
            boneData.matrixes[_sampleParam.clipIdx][_sampleParam.currFrameIdx] = new PosRot() { position = tmpPos, rotation = tmpRot };
        }
    ...
    private void WriteToFile()
    {
        ...
        for (int i = 0; i < _sampleParams.Count; i++)
        {
            ...
            clipData.clipPixelOffset = dataIdx;
