...
public class GPUSkinSampler : MonoBehaviour {
    ...
    private void BeginSample() {
        ...
        for (int i = 0, imax = _clips.Length; i < imax; i++) {
            StartSampleClip((short)i);

            for (int j = 0; j < _sampleParam.frameCount; j++) {
        ...
        for (int i = 0; i < _sampleParams.Count; i++) {
            ...
            for (int j = 0; j < frameCnt; j++) {
                foreach (var boneData in _allBoneDatas) {
                    ...
                    pixels[pixelIdx++] = new Color(r.x, r.y, r.z, r.w);
                    pixels[pixelIdx++] = QuatTrans2UDQ(r, new Vector3(matrix.m03, matrix.m13, matrix.m23));
    ...
    Color QuatTrans2UDQ(Quaternion q0, Vector3 t) {
        // non-dual part (just copy q0):
        Color result = new Color(0, 0, 0, 0);

        // dual part:
        result.r = -0.5f * (t.x * q0.y + t.y * q0.z + t.z * q0.w);
        result.g = 0.5f * (t.x * q0.x + t.y * q0.w - t.z * q0.z);
        result.b = 0.5f * (-t.x * q0.w + t.y * q0.x + t.z * q0.y);
        result.a = 0.5f * (t.x * q0.z - t.y * q0.y + t.z * q0.x);
